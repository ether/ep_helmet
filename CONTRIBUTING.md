# Contributing to ep_helmet

Thanks for helping improve `ep_helmet` — a plugin for [Etherpad](https://etherpad.org/). LLM/Agent contributions are explicitly welcome, provided they follow the rules below.
...
`ep_helmet` is a plugin, so you develop it inside a checkout of [ether/etherpad](https://github.com/ether/etherpad).
...
# Install ep_helmet from your local clone
pnpm run plugins i --path ../ep_helmet

# Start the dev server (port 9001)
pnpm --filter ep_etherpad-lite run dev
```

Edits in `../ep_helmet/` are picked up after a server restart.
...
* Etherpad version, ep_helmet version, browser + OS
* Steps to reproduce
